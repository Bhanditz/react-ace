...
export default class SplitComponent extends Component {
  ...
  componentDidMount() {
    ...
    this.splitEditor.getSession().setAnnotations(annotations || []);
  ...
  componentWillReceiveProps(nextProps) {
    ...
    split.forEach((editor) => {
      ...
      if (!isEqual(nextProps.annotations, oldProps.annotations)) {
        editor.getSession().setAnnotations(nextProps.annotations || []);
      }
      for (let i = 0; i < editorOptions.length; i++) {
        const option = editorOptions[i];
  ...
  handleMarkers(markers) {
    ...
    let currentMarkers = this.splitEditor.getSession().getMarkers(true);
    for (const i in currentMarkers) {
      if (currentMarkers.hasOwnProperty(i)) {
        this.splitEditor.getSession().removeMarker(currentMarkers[i].id);
      }
    }
    // remove background markers
    currentMarkers = this.splitEditor.getSession().getMarkers(false);
    for (const i in currentMarkers) {
      if (currentMarkers.hasOwnProperty(i)) {
        this.splitEditor.getSession().removeMarker(currentMarkers[i].id);
      }
    ...
    markers.forEach(({ startRow, startCol, endRow, endCol, className, type, inFront = false }) => {
      ...
      this.splitEditor.getSession().addMarker(range, className, type, inFront);
    });
  }
